$(document).ready(function() {
    ...
    $.ajax({
      url: 'http://127.0.0.1:8000/v1/scraper/number_of_ads',
      type: 'GET',
  ...
  $(document).ready(function() {
    // Perform AJAX request to fetch the dynamic data
    $.ajax({
      url: 'http://127.0.0.1:8000/v1/scraper/ad_queries_count',
      type: 'GET',
      success: function(response) {
        var numberOfAds = response.data;
   ...
   function fetchData() {
    ...
    xhr.open("GET", "http://127.0.0.1:8000/v1/scraper", true);
...
function populateTable(data) {
  ...
  data.forEach(function (project) {
    ...
    <a class="btn btn-info btn-sm" href="http://127.0.0.1:8000/crawler/${project.ad_id}">
  ...
  function deleteAd(adId, row) {
    ...
    fetch(`http://127.0.0.1:8000/v1/crawler/delete_ad/${adId}`, {
      method: "DELETE",
      headers: {
